...
static const char *__outline_frag_shader =
    ...
    "\n"
    "	vec2 tl_uv = uv + vec2(-texel.x * outlineWidth, -texel.y * "
    "outlineWidth);\n"
    "	tl_uv.x = clamp(tl_uv.x, 0.0, 1.0);\n"
    "	tl_uv.y = clamp(tl_uv.y, 0.0, 1.0);\n"
    "\n"
    "	vec2 tr_uv = uv + vec2(texel.x * outlineWidth, -texel.y * "
    "outlineWidth);\n"
    "	tr_uv.x = clamp(tr_uv.x, 0.0, 1.0);\n"
    "	tr_uv.y = clamp(tr_uv.y, 0.0, 1.0);\n"
    "\n"
    "	vec2 bl_uv = uv + vec2(-texel.x * outlineWidth, texel.y * "
    "outlineWidth);\n"
    "	bl_uv.x = clamp(bl_uv.x, 0.0, 1.0);\n"
    "	bl_uv.y = clamp(bl_uv.y, 0.0, 1.0);\n"
    "\n"
    "	vec2 br_uv = uv + vec2(texel.x * outlineWidth, texel.y * "
    "outlineWidth);\n"
...
// NOTE: Keyboard
#define key_pressed_or_repeated(key)                                           \
...
bool init_sprite_from_sheet(Sprite *spr, Texture tex, Vector2i offset,
                            Vector2i size, size_t hframes, size_t vframes);
...
void draw_sprite_outlined_at(Sprite *spr, Vector2 pos, float outline_width,
                             Color outline_color, Color tint);
...
Textbox make_textbox(Font font, int fs, Color active_color,
                     Color inactive_color, Vector2 pos, Vector2 size,
                     size_t buff_size, const char *name, char ignore_char);
...
void UI_Draw_element_stack_push(UI_Draw_element_stack *stack,
                                UI_Draw_element val);
bool UI_Draw_element_stack_pop(UI_Draw_element_stack *stack,
                               UI_Draw_element *popped);
...
#define LAYOUTS_CAP 256
  ...
  Font *font;
  Rectangle bg_rect;
  Rectangle ui_rect; // @NOTE Rect covering the ui components (without
                     // bg_padding, etc)
  Vector2 pos;
  Vector2 pos_offset;
  bool is_moving;
...
UI UI_make(UI_Theme theme, Font *font, Vector2 pos, cstr title,
           Vector2 *mpos_ptr);
...
bool UI_dropdown(UI *this, int *selected, const char **items,
                 size_t items_count, bool *dropdown_expanded, int font_size,
                 Color color);
void UI_background(UI *this);
// @NOTE: We are defering drawing because we need to call UI funcs before any
// input handling for the frame is happened, if we want input ignoring. We just
// push draw info to a stack when the UI funcs are called and draw all of them
// at once on UI_draw().
// @NOTE: @IMPORTANT: IT IS A MUST TO CALL THIS BEFORE
// UI_end()!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
bool rectangles_intersect(Rectangle rect1, float rect1_rot, Rectangle rect2,
                          float rect2_rot);
// bool rect_resolve_rect_collision(Rectangle* rect1, const Rectangle rect2);
// void rect_get_3d_points(Rectangle rect, Vector3f* p0, Vector3f* p1, Vector3f*
// p2, Vector3f* p3); void rect_get_points(Rectangle rect, Vector2* p0, Vector2*
// p1, Vector2* p2, Vector2* p3);

// NOTE: Window
bool init_window(int screen_width, int screen_height, float scl,
                 const char *title, int *width_out, int *height_out);
...
void draw_texture_outlined(Texture2D texture, Vector2 pos, float outline_width,
                           Color outline_color, Color tint);
void draw_texture_centered(Texture2D tex, Vector2 pos, Color tint);
void begin_frame(void);
void end_frame(void);
void draw_ren_tex(RenderTexture2D ren_tex);
void draw_text_aligned(Font font, const char *text, Vector2 pos, int font_size,
                       const Text_align_v align_v, const Text_align_h align_h,
                       Color color);
void draw_text_aligned_ex(Font font, const char *text, Vector2 pos,
                          int font_size, const Text_align_v align_v,
                          const Text_align_h align_h, float rotation,
                          Color color);
void draw_text(Font font, const char *text, Vector2 pos, int font_size,
               Color color);
...
const char *config_value_kind_as_str(const Config_value_kind k);
typedef struct Config_value Config_value;
...
bool set_col_to_config(Config *config, const char *key, Color value);
...
bool load_texture_(Asset_manager *am, const char *filepath, Texture2D *tex_out,
                   bool verbose);
bool load_texture_from_data(Asset_manager *am, const char *name,
                            unsigned char *data, int data_size,
                            Texture2D *tex_out);
bool load_texture_from_data_(Asset_manager *am, const char *name,
                             unsigned char *data, int data_size,
                             Texture2D *tex_out, bool verbose);
bool load_sound(Asset_manager *am, const char *filepath, Sound *s_out);
bool load_sound_(Asset_manager *am, const char *filepath, Sound *s_out,
                 bool verbose);
...
Nine_slice_box load_nine_slice_box(Asset_manager *am, const char *texpath,
                                   int tile_size, Vector2 tl, Vector2 tr,
                                   Vector2 bl, Vector2 br, Vector2 t, Vector2 r,
                                   Vector2 b, Vector2 l, Color fill_color);
...
void add_line_to_console_simple(Console *console, char *line, Color color,
                                bool hist);
void add_line_to_console(Console *console, char *buff, size_t buff_size,
                         Color color, bool histt);
void add_line_to_console_prefixed(Console *console, Arena *tmp_arena,
                                  char *buff, Color color, bool histt);
...
bool input_to_console(Console *console, char *ignore_characters,
                      size_t ignore_characters_count);
float get_cursor_offset(Console *console, int font_size);
void draw_console(Console *console, Rectangle rect, Vector2 pad,
                  Color fill_color, Color border_color, float alpha);
void console_prompt(Console *console, const char *prompt,
                    String_array *expected_prompt_values);
Ids match_command(const char *command, const char **commands,
...
#ifdef DEBUG
#define log_debug_console(console, fmt, ...)                                   \
  do {                                                                         \
    Console_line l = {                                                         \
        .color = YELLOW,                                                       \
    };                                                                         \
...
void cam_control(Cam *cam, float delta) {
  ...
  // Clamp view down
  float maxAngleDown =
      Vector3Angle(Vector3Negate(cam->cam3d.up), cam->cam3d.target);
  ...
  if (IsKeyDown(KEY_A)) {
    ...
    Vector3 left =
        Vector3Normalize(Vector3CrossProduct(cam->cam3d.up, forward));

    cam->cam3d.position = Vector3Add(
        cam->cam3d.position, Vector3Scale(left, cam->move_speed * delta));
  }
  if (IsKeyDown(KEY_D)) {
    Vector3 forward = cam_forward(cam);
    Vector3 left =
  ...
  if (!IsKeyDown(KEY_LEFT_ALT) && IsKeyDown(KEY_W)) {
    Vector3 forward = cam_forward(cam);
    cam->cam3d.position = Vector3Add(
        cam->cam3d.position, Vector3Scale(forward, cam->move_speed * delta));
  }
...
void cam_first_person_update(Cam *cam) {
  ...
  Vector3 up = {
      .x = world_up.x * cr +
           (forward.y * world_up.z - forward.z * world_up.y) * sr,
      .y = world_up.y * cr +
           (forward.z * world_up.x - forward.x * world_up.z) * sr,
      .z = world_up.z * cr +
           (forward.x * world_up.y - forward.y * world_up.x) * sr,
  };
...
bool mouse_button_down_unignored(int btn) { return IsMouseButtonDown(btn); }

bool mouse_button_released_unignored(int btn) {
  return IsMouseButtonReleased(btn);
}

void ignore_mouse_input(bool ignore) { __eng_ignore_mouse_input = ignore; }
...
Vector2 v2_from_degrees(float d) { return v2_from_radians(DEG2RAD * d); }
...
float v2_degrees(Vector2 v) { return v2_radians(v) * RAD2DEG; }
...
bool v2i_equal(Vector2i a, Vector2i b) { return a.x == b.x && a.y == b.y; }
...
void draw_nine_slice_box(Nine_slice_box *nbox, Vector2 pos, Vector2i size) {
  // Top Left
  draw_tex_rect(nbox->tex, pos, v2_scale(nbox->tl, nbox->tile_size),
                v2xx(nbox->tile_size));

  // Top Right
  draw_tex_rect(nbox->tex, v2(pos.x + (size.x * nbox->tile_size), pos.y),
                v2_scale(nbox->tr, nbox->tile_size), v2xx(nbox->tile_size));

  // Bottom Left
  draw_tex_rect(nbox->tex, v2(pos.x, pos.y + (size.y * nbox->tile_size)),
                v2_scale(nbox->bl, nbox->tile_size), v2xx(nbox->tile_size));

  // Bottom Right
  draw_tex_rect(nbox->tex,
                v2(pos.x + (size.x * nbox->tile_size),
                   pos.y + (size.y * nbox->tile_size)),
                v2_scale(nbox->br, nbox->tile_size), v2xx(nbox->tile_size));

  // Top
  for (int i = 1; i < size.x; ++i) {
    draw_tex_rect(nbox->tex, v2(pos.x + (i * nbox->tile_size), pos.y),
                  v2_scale(nbox->t, nbox->tile_size), v2xx(nbox->tile_size));
  }
  ...
  for (int i = 1; i < size.y; ++i) {
    draw_tex_rect(
        nbox->tex,
        v2(pos.x + (size.x * nbox->tile_size), pos.y + (i * nbox->tile_size)),
        v2_scale(nbox->r, nbox->tile_size), v2xx(nbox->tile_size));
  }
  ...
  for (int i = 1; i < size.x; ++i) {
    draw_tex_rect(
        nbox->tex,
        v2(pos.x + (i * nbox->tile_size), pos.y + (size.y * nbox->tile_size)),
        v2_scale(nbox->b, nbox->tile_size), v2xx(nbox->tile_size));
  }

  // Left
  for (int i = 1; i < size.y; ++i) {
    draw_tex_rect(nbox->tex, v2(pos.x, pos.y + (i * nbox->tile_size)),
                  v2_scale(nbox->l, nbox->tile_size), v2xx(nbox->tile_size));
  }
...
// NOTE: UI
// @TODO: make an UI array struct and do begin/end and draws on them, make the
// active UI item the last in the array so that way the active UI item has the
// input.
...
static Vector2 get_ui_bg_rect_pos(UI *this) {
  return (Vector2){
      ...
      .y = this->pos.y +
           (this->theme.titlebar_height *
            (1.f + this->theme.titlebar_pad_bottom)) -
           this->theme.bg_padding.y,
  };
}
...
void set_ui_theme_titlebar_font_size(UI_Theme *theme, int font_size) {
  ...
  theme->titlebar_height =
      (float)(theme->titlebar_font_size + (theme->titlebar_padding * 2.f));
}
...
UI_Draw_element_stack UI_Draw_element_stack_make(void) {
  ...
  res.buff = (UI_Draw_element *)arena_alloc(
      &res.arena, sizeof(UI_Draw_element) * draw_element_count);
...
void UI_begin(UI *this, UI_Layout_kind kind) {
  __eng_ignore_mouse_input = false;
  UI_Layout layout = {0};
  layout.pos = Vector2Add(
      this->pos, (Vector2){0.f, this->theme.titlebar_height *
                                    (1 + this->theme.titlebar_pad_bottom)});
...
bool UI_button(UI *this, cstr text, int font_size, Color color) {
  ...
  const Vector2 size =
      Vector2Add(MeasureTextEx(*this->font, text, font_size, 1.f),
                 Vector2Scale(this->btn_padding, 2.f));
  ...
  bool is_clicked =
      (hovering && mouse_button_down_unignored(MOUSE_BUTTON_LEFT));
  if (is_clicked) {
    alpha = 1.f;
  }

  UI_Draw_element_stack_push(&this->draw_element_stack,
                             (UI_Draw_element){
...
void UI_sprite(UI *this, Sprite *spr) {
  ...
  const Vector2 pos = UI_Layout_available_pos(top);
  const Vector2 size = (Vector2){spr->tex_rect.width * spr->scale.x,
                                 spr->tex_rect.height * spr->scale.y};
...
bool UI_dropdown(UI *this, int *selected, const char **items,
                 size_t items_count, bool *expanded, int font_size,
                 Color color) {
  ...
  if (*expanded) {
    for (int i = 0; i < items_count; ++i) {
      ...
      Vector2 size =
          Vector2Add(MeasureTextEx(*this->font, item, font_size, 1.f),
                     Vector2Scale(this->btn_padding, 2.f));
    ...
    Vector2 size = Vector2Add(MeasureTextEx(*this->font, item, font_size, 1.f),
                              Vector2Scale(this->btn_padding, 2.f));
    Rectangle rect = {
        .x = pos.x,
        .y = pos.y,
...
static void UI_titlebar(UI *this) {
  Vector2 rect_pos =
      Vector2Subtract(this->pos, (Vector2){this->theme.bg_padding.x, 0.f});
  Vector2 rect_size =
      v2(this->bg_rect.width, fmaxf(this->theme.titlebar_height,
                                    this->theme.titlebar_font_size +
                                        (2 * this->theme.titlebar_padding)));
  Rectangle titlebar = {
      .x = rect_pos.x,
      .y = rect_pos.y,
      .width = rect_size.x,
  ...
  Vector2 title_pos = v2(titlebar.x + this->theme.titlebar_padding,
                         titlebar.y + this->theme.titlebar_padding);
  /* Vector2 ui_size = get_ui_size(this); */
  DrawTextEx(*this->font, TextFormat("%s", this->title), title_pos,
             this->theme.titlebar_font_size, 1.f, WHITE);
  /* DrawTextEx(*this->font, TextFormat("%s %d, %d | %d, %d", this->title,
   * (int)this->scroll_offset.x, (int)this->scroll_offset.y, (int)ui_size.x,
   * (int)ui_size.y), title_pos, this->theme.titlebar_font_size, 1.f, WHITE); */
}
...
void UI_draw(UI *this) {
  ...
  BeginScissorMode(this->bg_rect.x, this->bg_rect.y, this->bg_rect.width,
                   this->bg_rect.height);
  ...
  while (UI_Draw_element_stack_pop(&this->draw_element_stack, &elm)) {
    ...
    case UI_DRAW_ELEMENT_TYPE_BOX: {
      ...
      DrawRectangleLinesEx(
          (Rectangle){elm.pos.x, elm.pos.y, elm.size.x, elm.size.y}, 1.f,
          elm.out_color);
    } break;
    case UI_DRAW_ELEMENT_TYPE_SPRITE: {
    ...
    case UI_DRAW_ELEMENT_TYPE_TEXT: {
      DrawTextEx(*elm.font, elm.text, elm.pos, elm.font_size, 1.f,
                 elm.fill_color);
    } break;
    case UI_DRAW_ELEMENT_TYPE_LINE: {
...
void UI_end(UI *this) {
  ...
  Vector2 title_size =
      (Vector2){this->bg_rect.width, this->theme.titlebar_height};
  Rectangle titlebar = {
      .x = title_pos.x,
      .y = title_pos.y,
      .width = title_size.x,
  ...
  if (mouse_button_pressed_unignored(MOUSE_BUTTON_LEFT) &&
    ...
    /* this->pos_offset = Vector2Subtract(Vector2Subtract(mpos, title_pos),
     * (Vector2) {this->theme.bg_padding.x, 0.f}); */
  ...
  if ((this->show) &&
      (mouse_button_down_unignored(MOUSE_BUTTON_LEFT) ||
       mouse_button_down_unignored(MOUSE_BUTTON_MIDDLE) ||
       mouse_button_down_unignored(MOUSE_BUTTON_RIGHT)) &&
...
// Sprite
bool init_sprite(Sprite *spr, Texture2D texture, size_t hframes,
                 size_t vframes) {
  spr->texture = texture;
  spr->hframes = hframes;
  spr->vframes = vframes;
...
void draw_sprite(Sprite *spr) {
  ...
  DrawTexturePro(spr->texture, spr->tex_rect, dest, origin, spr->rotation,
                 spr->tint);
...
void draw_sprite_outlined_at(Sprite *spr, Vector2 pos, float outline_width,
  ...
  float texture_size[2] = {(float)__temp_ren_tex.texture.width,
                           (float)__temp_ren_tex.texture.height};
  SetShaderValue(__outline_shader, __outline_shader_texture_size_loc,
                 texture_size, SHADER_UNIFORM_VEC2);
  SetShaderValue(__outline_shader, __outline_shader_outline_width_loc,
                 &outline_width, SHADER_UNIFORM_FLOAT);
  ...
  SetShaderValue(__outline_shader, __outline_shader_outline_color_loc,
                 &outline_color_, SHADER_UNIFORM_VEC4);
...
void free_sprite(Sprite *spr) { (void)spr; }

void set_sprite_scale_scalar(Sprite *spr, float scl) { spr->scale = v2xx(scl); }
...
bool input_to_textbox(Textbox *tbox) {
  if (!tbox->active)
    return false;
  if (input_to_buff_ignored(tbox->buff, tbox->buff_size, &tbox->cursor,
                            tbox->ignore_char, &tbox->ignoring_input)) {
    tbox->ignoring_input = true;
    return true;
  }
  return false;
...
Rectangle get_textbox_rect(Textbox *tbox) {
  float buff_measured =
      MeasureTextEx(tbox->font, tbox->buff, tbox->font_size, 1.f).x;
  float name_measured =
      MeasureTextEx(tbox->font, tbox->name, tbox->font_size, 1.f).x;
...
void draw_textbox(Textbox *tbox) {
  ...
  draw_text(tbox->font, tbox->name, tbox->pos, tbox->font_size,
            tbox->active ? tbox->active_color : tbox->inactive_color);
  draw_text(tbox->font, tbox->buff, v2(rect.x + 2, tbox->pos.y),
            tbox->font_size,
            tbox->active ? tbox->active_color : tbox->inactive_color);
  DrawRectangleLinesEx(
      rect, 1, tbox->active ? tbox->active_color : tbox->inactive_color);
}

// Rectangle
bool rect_contains_point(Rectangle r1, Vector2 p) {
  return (p.x >= r1.x && p.x < r1.x + r1.width && p.y >= r1.y &&
          p.y < r1.y + r1.height);
}

bool rect_contains_rect(Rectangle r1, Rectangle r2) {
  return (
      rect_contains_point(r1, v2(r2.x, r2.y)) &&
      rect_contains_point(r1, (Vector2){r2.x + r2.width, r2.y + r2.height}));
}
...
bool rect_intersects_rect(Rectangle r1, Rectangle r2) {
  ...
  return (rect1_r >= rect2_l && rect1_l <= rect2_r && rect1_t <= rect2_b &&
          rect1_b >= rect2_t);
}
...
void rectangle_corners(Rectangle rect, float rot, Vector2 corners[4]) {
  ...
  Vector2 local_corners[4] = {{-hw, -hh}, {-hw, hh}, {hw, hh}, {hw, -hh}};
...
// }
//
// void rect_get_points(Rectangle rect, Vector2* p0, Vector2* p1, Vector2* p2,
// Vector2* p3) {
...
bool init_window(int screen_width, int screen_height, float scl,
  ...
  *height_out = height;
  ...
  log_info(
      "Created RenderTexture2D with dimensions %dx%d (Scaled down by %.2f)",
      __ren_tex.texture.width, __ren_tex.texture.height, scl);

  // Load pre-defined shaders
  __outline_shader =
      LoadShaderFromMemory(__outline_vert_shader, __outline_frag_shader);
  ...
  __outline_shader_texture_size_loc =
      GetShaderLocation(__outline_shader, "textureSize");
  __outline_shader_outline_width_loc =
      GetShaderLocation(__outline_shader, "outlineWidth");
  __outline_shader_outline_color_loc =
      GetShaderLocation(__outline_shader, "outlineColor");
...
void draw_texture_centered(Texture2D tex, Vector2 pos, Color tint) {
  ...
  DrawTexturePro(tex, src, dst, v2(tex.width * 0.5, tex.height * 0.5), 0.f,
                 tint);
}
void begin_frame(void) {
  BeginDrawing();
...
void draw_ren_tex(RenderTexture2D ren_tex) {
  const Rectangle src = {
      ...
      // NOTE: We flip the height because y-axis is flipped internally (in
      // opengl land probably)
  ...
  DrawTexturePro(ren_tex.texture, src, dst, CLITERAL(Vector2){0.f, 0.f}, 0.f,
                 WHITE);
}
...
void draw_text_aligned(Font font, const char *text, Vector2 pos, int font_size,
                       const Text_align_v align_v, const Text_align_h align_h,
                       Color color) {
  draw_text_aligned_ex(font, text, pos, font_size, align_v, align_h, 0.0,
                       color);
}
...
void draw_text(Font font, const char *text, Vector2 pos, int font_size,
               Color color) {
  draw_text_aligned(font, text, pos, font_size, TEXT_ALIGN_V_TOP,
                    TEXT_ALIGN_H_LEFT, color);
}
...
Config_value config_value_from_sv(String_view sv, Config *config) {
  ...
  if (sv.count <= 0) {
    res.err_msg = "Value is empty";
    return res;
  }

  if (sv.data[0] == '\'') {
    // char
    res.kind = CONF_VAL_CHR;
    String_view ch = sv;
    sv_lremove(&ch, 1); // Remove '

    res.as.ch = ch.data[0];

    sv_lremove(&ch, 1); // Remove char

    if (ch.count <= 0 || ch.data[0] != '\'') {
      res.err_msg = "Unterminated char: Expceted '";
      return res;
    }
  } else if (sv.data[0] == '"') {
    // string
    res.kind = CONF_VAL_STR;
    ...
    if (sv_contains_char(sv, '.')) {
      // float
      res.kind = CONF_VAL_FLT;

      int count = -1;
      res.as.f = c_sv_to_float(sv, &count);
      if (count == -1) {
        res.err_msg = "Failed to convert to float";
      }
    } else {
      // integer
      res.kind = CONF_VAL_INT;

      int count = -1;
      res.as.i = c_sv_to_int(sv, &count, 10);
      if (count == -1) {
        res.err_msg = "Failed to convert to integer";
      }
    }
  } else if (sv_starts_with(sv, "rgba")) {
    // @TODO: More proper error checking.
    // color
    res.kind = CONF_VAL_COL;
    ...
    sv_lremove(&sv, 4); // Remove rgba
    sv_ltrim(&sv);

    if (sv.count == 0) {
      res.err_msg = "Expected ( after 'rgba' but got nothing";
      return res;
    } else if (sv.data[0] != '(') {
      res.err_msg = "Expected ( after 'rgba'";
      return res;
    }
    ...
    if (!sv.count || sv.data[0] == ')') {
      res.err_msg = "Expected R after (";
      return res;
    }

    r_sv = sv_lpop_until_char(&sv, ',');

    if (!sv.count || sv.data[0] != ',') {
      res.err_msg = "Expected , after R";
      return res;
    }
    ...
    g_sv = sv_lpop_until_char(&sv, ',');

    if (!sv.count || sv.data[0] != ',') {
      res.err_msg = "Expected , after G";
      return res;
    }
    ...
    b_sv = sv_lpop_until_char(&sv, ',');

    if (!sv.count || sv.data[0] != ',') {
      res.err_msg = "Expected , after B";
      return res;
    }
    ...
    a_sv = sv_lpop_until_char(&sv, ')');

    if (!sv.count || sv.data[0] != ')') {
      res.err_msg = "Expected ) after A";
      return res;
    }
    ...
    {
      int count = -1;
      res.as.color.r = c_sv_to_int(r_sv, &count, 10);
      if (count == -1) {
        res.err_msg = "Failed to convert R to integer";
      }
    }
    {
      int count = -1;
      res.as.color.g = c_sv_to_int(g_sv, &count, 10);
      if (count == -1) {
        res.err_msg = "Failed to convert G to integer";
      }
    }
    {
      int count = -1;
      res.as.color.b = c_sv_to_int(b_sv, &count, 10);
      if (count == -1) {
        res.err_msg = "Failed to convert B to integer";
      }
    }
    {
      int count = -1;
      res.as.color.a = c_sv_to_int(a_sv, &count, 10);
      if (count == -1) {
        res.err_msg = "Failed to convert A to integer";
      }
    }

  } else {
    // var
    res.kind = CONF_VAL_VAR;
...
const char *config_value_as_str(Arena *str_arena, const Config_value cv) {
  ASSERT(str_arena, "str_arena should be valid arena");
  const char *res = NULL;
  switch (cv.kind) {
  case CONF_VAL_INT: {
    res = c_arena_alloc_str(*str_arena, "%d", cv.as.i);
  } break;
  case CONF_VAL_FLT: {
    res = c_arena_alloc_str(*str_arena, "%.2f", cv.as.f);
  } break;
  case CONF_VAL_CHR: {
    res = c_arena_alloc_str(*str_arena, "%c", cv.as.ch);
  } break;
  case CONF_VAL_STR: {
  ...
  case CONF_VAL_COL: {
    res = c_arena_alloc_str(*str_arena, "rgba(%d, %d, %d, %d)", cv.as.color.r,
                            cv.as.color.g, cv.as.color.b, cv.as.color.a);
  } break;
  case CONF_VAL_COUNT:
  default:
    ASSERT(false, "UNREACHABLE!");
...
// @TODO: Incorrect line number when there are empty lines.
bool read_config(Config *config, const char *config_filepath) {
  int f_size = -1;
  const char *f = read_file(config_filepath, &f_size);
  if (f_size == -1) {
    return false;
  }
  ...
  while (sv.count > 0) {
    ...
    sv_trim(&sv);
    String_view line = c_sv_lpop_until_char(&sv, '\n');
    sv_trim(&line); // This will remove \n AND \r too (i hope)
    ...
    if (line.count > 0 && line.data[0] == '#') {
      ...
      // Parse key
      String_view key_sv = c_sv_lpop_until_char(&line, '=');
      sv_trim(&key_sv);
      sv_lremove(&line, 1); // Remove =
      sv_trim(&line);

      // Parse value
      String_view value_sv = line;
...
bool write_config(Config *config, const char *config_filepath) {
  ...
  for (int i = 0; i < keyvalues_count; ++i) {
    ...
    case CONF_VAL_COL: {
      fprintf(f, "rgba(%d, %d, %d, %d)", value.as.color.r, value.as.color.g,
              value.as.color.b, value.as.color.a);
    } break;
    case CONF_VAL_COUNT:
    default:
      ASSERT(false, "UNREACHABLE!");
...
bool set_col_to_config(Config *config, const char *key, Color value) {
  ...
  log_debug("Key '%s' found, changing from rgba(%d, %d, %d, %d)"
            "-> rgba(%d, %d, %d, %d)...",
            key, config_kv->value.as.color.r, config_kv->value.as.color.g,
            config_kv->value.as.color.b, config_kv->value.as.color.a, value.r,
            value.g, value.b, value.a);

  config_kv->value.kind = CONF_VAL_COL;
  config_kv->value.as.color = value;
  if (config_kv->value.value_ptr) {
    *((Color *)(config_kv->value.value_ptr)) = value;
  }
...
bool set_value_to_config(Config *config, const char *key, Config_value value) {
  ...
  case CONF_VAL_COL:
    return set_col_to_config(config, key, value.as.color);
  case CONF_VAL_COUNT:
  default:
    ASSERT(false, "UNREACHABLE");
  }
...
bool load_texture_(Asset_manager *am, const char *filepath, Texture2D *tex_out,
  ...
  if (tex_KV != NULL) {
    ...
    if (verbose) {
      log_debug("Found '%s' at texture_map index [%zu]", filepath,
                shlenu(am->texture_map));
    }
  } else {
    ...
    if (verbose) {
      log_debug("Added '%s' to texture_map index [%zu]", filepath,
                shlenu(am->texture_map));
    }
  }
...
bool load_texture_from_data_(Asset_manager *am, const char *name,
  ...
  if (tex_KV != NULL) {
    ...
    if (verbose) {
      log_debug("Found '%s' at texture_map index [%zu]", name,
                shlenu(am->texture_map));
    }
  } else {
    ...
    if (verbose) {
      log_debug("Added '%s' (from memory) to texture_map index [%zu]", name,
                shlenu(am->texture_map));
    }
  }
...
bool load_sound_(Asset_manager *am, const char *filepath, Sound *s_out,
  ...
  if (sound_KV != NULL) {
    ...
    if (verbose) {
      log_debug("Found '%s' at sound_map index [%zu]", filepath,
                shlenu(am->sound_map));
    }
  } else {
    ...
    if (verbose) {
      log_debug("Added '%s' to sound_map index [%zu]", filepath,
                shlenu(am->sound_map));
    }
  }
...
void add_line_to_console_prefixed(Console *console, Arena *tmp_arena,
                                  char *buff, Color color, bool hist) {
  const char *prefixed =
      arena_alloc_str(*tmp_arena, "%s%s%c%s", console->prefix, console->prefix2,
                      console->prefix_symbol, buff);
...
Console_line *get_console_line(Console *console, size_t line) {
  if (line >= console->lines.count) {
    log_error("Outofbounds: %zu is out of bounds of lines.count (%zu)", line,
              console->lines.count);
    return NULL;
  }
...
Console_line *get_console_history(Console *console, size_t line) {
  if (line >= console->history.count) {
    log_error("Outofbounds: %zu is out of bounds of history.count (%zu)", line,
...
char *get_current_console_line_buff(Console *console) {
  ...
  if (console->line >= console->lines.count) {
    log_error("Outofbounds: %d is out of bounds of lines.count (%zu)",
              console->line, console->lines.count);
    return NULL;
  }
...
void readline_update(Console *console, Console_line *line) {
  Console_line *last_line =
      get_console_history(console, console->hist_lookup_idx);
  bool should_get_unprefixed_lines =
      GET_FLAG(console->flags, CONSOLE_FLAG_READLINE_USES_UNPREFIXED_LINES);
  if (should_get_unprefixed_lines) {
    if (console->hist_lookup_idx >= console->unprefixed_lines.count) {
      log_error(
          "Outofbounds: %d is out of bounds of unprefixed_lines.count (%zu)",
          console->hist_lookup_idx, console->unprefixed_lines.count);
      return;
    }
...
bool input_to_console(Console *console, char *ignore_characters,
  ...
  if (console->cursor < 0)
    console->cursor = 0;
  if (console->cursor > CONSOLE_LINE_BUFF_CAP - 1)
    console->cursor = CONSOLE_LINE_BUFF_CAP - 1;
  ...
  do {
      ...
      if (console->prompting && console->expecting_values) {
        ...
        if (!found) {
          ...
          for (int i = 0; i < console->expected_prompt_values.count; ++i) {
            log_error_console((*console), "		- %s",
                              console->expected_prompt_values.items[i]);
          }
        }
    ...
    if (IsKeyDown(KEY_LEFT_CONTROL)) {
      ...
      if (IsKeyPressed(KEY_N)) {
        if (console->lines.count > 0) {
          ...
          // log_info("%d (%s)", console->hist_lookup_idx,
          // console->hist_lookup_idx < console->history.count-2 ? "true" :
          // "false");
          readline_update(console, line);
        }
      }
    ...
    if (ch > 0) {
      // log_debug("TYPED %c AT %d:%d", (char)ch, console->line,
      // console->cursor); log_debug("CODEPOINT %c: %fx%f", ch,
      // codepoint_rec.width, codepoint_rec.height);
...
float get_cursor_offset(Console *console, int font_size) {
  ...
  for (int i = 0; i < console->cursor && text[i] != '\0';) {
    ...
    // GlyphInfo glyph = GetGlyphInfo(font, codepoint); // pseudo: access
    // font.glyphs[...] or use raylib helpers
    i += 1;
    Rectangle glyph_atlas_rec = GetGlyphAtlasRec(font, codepoint);
    x += glyph_atlas_rec.width * scale;

    // log_info("%c: %f, %f, %fx%f", codepoint, glyph_atlas_rec.x,
    // glyph_atlas_rec.y, glyph_atlas_rec.width, glyph_atlas_rec.height);
  }
...
void draw_console(Console *console, Rectangle rect, Vector2 pad,
  ...
  for (size_t i = 0; i < console->lines.count; ++i) {
    ...
    draw_text(GetFontDefault(), line->buff, pos, console->font_size,
              ColorAlpha(line->color, alpha));
  ...
  snprintf(actual_prefix, 1024, "%s%s%c",
           console->prefix ? console->prefix : "",
           console->prefix2 ? console->prefix2 : "", console->prefix_symbol);

  draw_text(console->font, actual_prefix,
            v2(rect.x + 4.f, rect.y + rect.height), console->font_size,
            ColorAlpha(WHITE, alpha));
  float prefix_offset =
      MeasureTextEx(console->font, actual_prefix, console->font_size, 2.5f).x +
      10.f;
  draw_text(console->font, get_current_console_line_buff(console),
            v2(rect.x + prefix_offset, rect.y + rect.height),
            console->font_size, ColorAlpha(WHITE, alpha));
...
void console_prompt(Console *console, const char *prompt,
  ...
  if (expected_prompt_values != NULL) {
    ...
    for (int i = 0; i < expected_prompt_values->count; ++i) {
      darr_append(console->expected_prompt_values,
                  expected_prompt_values->items[i]);
    }
  }
}
...
Ids match_command(const char *command, const char **commands,
  ...
  for (size_t i = 0; i < copy_len; ++i)
    command_lower[i] = tolower((unsigned char)command[i]);
  command_lower[copy_len] = 0;
  if (command_len > max)
    command_len = copy_len; // or treat as truncated
  //
...
// Timer and Alarm
void update_timer(Timer *t, float dt) { t->time += dt; }
